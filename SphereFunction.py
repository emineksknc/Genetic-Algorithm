...
def  fitnes_fun(population):
    ...
    for k in population: # kromozom sayisi 
        sum = sum + np.power(k,2)  
        y.append(np.power(k,2))
...
def RouletteWheelSelection(fitnessSum,fitnessValues,chromosome):
    ...
    fitnessValues = [np.divide(np.multiply(f,100),fitnessSum) for f in fitnessValues]
...
def SphereFunction(InitialPopulation,chromosome):
    ...
    for i in range(100):
        #yeni popülasyonun fitness değerlerinin bulunması
